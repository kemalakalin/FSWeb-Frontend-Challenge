...
export const Header = () => {
  // t objesini de buradan çekiyoruz
  const { isDarkMode, toggleTheme, lang, toggleLang, t } = useSite();
    ...
    <section className={`min-h-screen flex flex-col px-4 lg:px-48 transition-colors duration-300 ${isDarkMode ? 'bg-[#252128]' : 'bg-[#F4F4F4]'}`}>
      ...
      <div className="relative z-50 w-full flex justify-end items-center py-6 gap-4 font-bold text-sm tracking-wider">
        ...
        >
          ...
          <span className={`${isDarkMode ? 'text-[#D9D9D9]' : 'text-[#777777]'}`}>
            {t.darkMode}
          </span>
        </div>
        ...
        <div 
          ...
          onClick={toggleLang}
        >
          <span className="text-[#E92577]">{t.switchLanguage}</span>
          <span className="text-[#777777]">{t.switchText}</span>
        </div>
      </div>
      ...
      <div className="container mx-auto flex flex-col lg:flex-row items-center justify-between gap-12 flex-grow">
        ...
        <div className="flex-1 z-10">
          <p className={`text-2xl md:text-3xl mb-6 flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
            {t.hi} <span className="animate-bounce">👋</span>
          </p>
          
          <h1 className={`text-4xl md:text-5xl font-bold leading-tight max-w-2xl relative transition-colors ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
          ...
          <div className="flex gap-4 mt-12 mb-10">
            <a href="#" className="hover:opacity-75 transition-opacity bg-white p-2 rounded-lg shadow-md">
          ...
          <div className="space-y-6">
            <h3 className={`text-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
              {t.statusPrefix} <span className="text-[#E92577] font-medium">{t.statusRole}</span> {t.statusSuffix}
            </h3>
            <h3 className={`text-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
              {t.invite} 
